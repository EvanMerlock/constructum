...
use crate::{ConstructumServerState, server::error::ConstructumServerError, s3buckets};
use super::JobInfo;
...
pub async fn get_job_logs(
    ...
    let result = join_all(files_to_pull.into_iter().map(|file_name| s3buckets::get_file_from_s3(file_name, state.s3_bucket()))).await.into_iter().map(|x| x.expect("failed to grab s3 logs")).collect();
    Ok(Json(result))
}